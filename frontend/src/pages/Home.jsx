...
import { getMovies, semanticSearch } from '../services/api.js';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadMovies() {
      ...
      try {
        if (moodQuery.trim()) {
          const results = await semanticSearch(moodQuery.trim());
          if (cancelled) return;
          setMovies(results);
          setCacheSource('vector');
        } else {
        ...
        if (!cancelled) {
          ...
          setError(
            moodQuery.trim()
              ? 'Semantic search failed. Check the Hugging Face token, Atlas index, and backend.'
              : 'Could not load movies. Start the backend, then refresh.'
          );
        }
      } finally {
